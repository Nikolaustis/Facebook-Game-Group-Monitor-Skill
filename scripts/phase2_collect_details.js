...
function detectLanguageSignal(groupName, aboutText, snippet) {
  const t = clean(`${groupName || ''}\n${aboutText || ''}\n${snippet || ''}`);
  const lower = t.toLowerCase();

  const thaiChars = countThaiChars(t);
  const vnDia = countVnDiacritics(t);
  const zhChars = countChineseChars(t);
  const enChars = countEnglishLetters(t);
  const vnKeywords = (lower.match(/\b(vietnam|viet\s*nam|viet|vn|thanh\s*vien|bai\s*viet|nhom)\b/g) || []).length;
  const thaiKeywords = (lower.match(/\b(thailand|thai|th)\b/g) || []).length;

  const thaiScore = thaiChars * 2 + thaiKeywords * 6;
  const vnScore = vnDia * 2 + vnKeywords * 6;

  if (thaiScore >= 16 && thaiScore >= vnScore + 4) return 'Thai';
  if (vnScore >= 16 && vnScore >= thaiScore + 4) return 'Vietnamese';
  if (thaiScore >= 12 && vnScore < 12) return 'Thai';
  if (vnScore >= 12 && thaiScore < 12) return 'Vietnamese';

  const hasThai = thaiScore >= 8;
  const hasVn = vnScore >= 8;
  const hasZh = zhChars >= 12;
  const hasEn = enChars >= 40;
  const scripts = [hasThai, hasVn, hasZh, hasEn].filter(Boolean).length;
  if (scripts >= 2) return 'Mixed';
...
function defaultLanguageToRegion(languageSignal) {
  if (languageSignal === 'Thai') return 'TH';
  if (languageSignal === 'Vietnamese') return 'VN';
  if (languageSignal === 'Chinese') return 'CN';
  if (languageSignal === 'English') return 'GLOBAL';
  return '';
}
...
function matchGame(profile, groupName, aboutText, snippet) {
  ...
  if (negativeGroup && (!exactGroup || negativeGroup.length > exactGroup.length)) {
    return {
      ...
      review_reason: 'group_name命中更具体的兄弟游戏标题',
      manual_review: true,
    };
  }
  ...
  if (negativeGroup) {
    return {
      ...
      phrase: '',
      negative_hit: negativeGroup,
      review_reason: 'group_name命中兄弟游戏标题',
      manual_review: true,
    };
  }
  ...
  if (exactFull) {
    return {
      ...
      phrase: exactFull,
      negative_hit: '',
      review_reason: '仅full_text命中目标完整标题，默认进入人工复核',
      manual_review: true,
    };
  }
  ...
  if (ipRootHit) {
    return {
      ...
      phrase: '',
      negative_hit: '',
      review_reason: 'group_name命中IP大词根但未命中目标完整标题',
      manual_review: true,
    };
  }
...
function extractGroupSize(aboutText) {
  ...
  if (m) return toInt(m[1]);
  m = t.match(/([0-9][0-9,]*)\s*th[aà]nh\s*vi[êe]n/i);
  if (m) return toInt(m[1]);
  return '';
}
...
function extractTodayPosts(aboutText) {
  ...
  if (m) return toInt(m[1]);
  m = t.match(/b[aà]i\s+vi[eế]t\s+m[ơo]i(?:\s+h[oô]m\s+nay)?\s*[:\-]?\s*([0-9][0-9,+]*)/i);
  if (m) return toInt(m[1]);
  return '';
}
...
function extractWeekNewFans(aboutText) {
  ...
  if (m) return toInt(m[1]);
  m = t.match(/th[aà]nh\s+vi[êe]n\s+m[ơo]i.*tu[aầ]n.*[:\-]?\s*\+?\s*([0-9][0-9,+]*)/i);
  if (m) return toInt(m[1]);
  return '';
}
...
function actionReason(todayPosts, weekNewFans, existed, threshold) {
  const bits = [];
  if (todayPosts !== '' && Number(todayPosts) >= threshold) bits.push('today_posts达到阈值');
  if (weekNewFans !== '' && Number(weekNewFans) >= threshold) bits.push('week_new_fans达到阈值');
  if (existed === 'yes') bits.push('existed_last_month=yes');
  else if (existed === 'no') bits.push('existed_last_month=no');
  else bits.push('existed_last_month缺失');
  return bits.join('；');
}
...
function unavailableText(text) {
  ...
  return /this content isn't available|content unavailable|椤甸潰涓嶅彲鐢▅鍏у鐒℃硶椤ず|鏆傛椂鏃犳硶浣跨敤|鎵句笉鍒版鍐呭|鐒℃硶鏌ョ湅/.test(t);
}
...
(async () => {
  ...
  const languageToRegion = config.language_to_region && typeof config.language_to_region === 'object' ? config.language_to_region : {};
  const regionKeywords = config.region_keywords && typeof config.region_keywords === 'object' ? config.region_keywords : {};
  ...
  try {
    ...
    for (const g of gameEntries) {
      ...
      for (let i = 0; i < candidates.length; i++) {
        ...
        const about = await fetchAboutWithRetry(page, c.group_url, 2);
        if (!about.ok) {
          stats.about_failed++;
          continue;
        }
