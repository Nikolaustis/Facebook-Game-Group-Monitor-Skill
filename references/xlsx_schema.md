...
- `__region_source`：地区来源，例如 `country_keyword` / `country_flag` / `country_keyword_and_flag` / `region_keyword` / `country_keyword_same_business_region` / `region_keyword_same_business_region` / `language_map` / `about_location_country_keyword` / `about_location_city_keyword` / `about_location_region_keyword` / `keyword_conflict` / 留空。
